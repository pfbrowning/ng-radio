...
export class FavoriteStationsEffects {
  ...
  updateFavorite$ = createEffect(() =>
    this.actions$.pipe(
      ...
      mergeMap(({ station }) =>
        this.favoriteStationsService.updateFavorite(station.stationId, station).pipe(
          map(updated =>
    ...
    this.actions$.pipe(
      ...
      mergeMap(action =>
        this.favoriteStationsService.removeFavorite(action.stationId).pipe(
          map(() =>
            FavoriteStationsActions.removeFromFavoritesSucceeded({ stationId: action.stationId })
          ),
          catchError(error =>
            of(
