...
export const createAppInsightsServiceSpy = () =>
  jasmine.createSpyObj<AppInsightsService>('appInsightsService', ['initialize']);

export const createNotificationsServiceSpy = () =>
  jasmine.createSpyObj('notificationsServiceSpy', ['info', 'success', 'error', 'warn']);

export const createMessageServiceSpy = () => {
  const spy = jasmine.createSpyObj<MessageService>('messageServiceSpy', ['add']);
...
export const createAuthenticationFacadeSpy = () => {
  const spy = jasmine.createSpyObj<AuthenticationFacadeService>('authenticationFacade', [
    'logoutButtonClicked',
    'logInRedirect',
  ]);
...
export const createAuthenticationServiceSpy = () =>
  jasmine.createSpyObj<AuthenticationService>('authenticationService', ['logIn']);

export const createSleepTimerServiceSpy = () => {
  const spy = jasmine.createSpyObj<SleepTimerService>('sleepTimerService', ['setTimer']);
  spy.minutesToSleep$ = of(7);
  return spy;
};

export const createAudioElementServiceSpy = () =>
  jasmine.createSpyObj<AudioElementService>('audioElementServiceSpy', ['play']);

export const createEnvironmentServiceSpy = () =>
  jasmine.createSpyObj<EnvironmentService>('environmentService', ['isProduction']);
...
export const createPlayerBarFacadeSpy = () =>
  jasmine.createSpyObj<PlayerBarFacadeService>('playerBarFacade', ['addToFavoritesClicked']);
