...
describe('NowPlayingComponent', () => {
  ...
  nowPlayingTemplateInput.forEach(station => {
    it('should update the template to reflect changes in station title', () => {
      ...
      expect(getElementTextBySelector<NowPlayingComponent>(fixture, '.station-title')).toBe(
        station.title
      );
    });
  });
  ...
  it('should update the template to reflect changes in minutes until sleep', () => {
    ...
    for (let i = 300; i >= 0; i--) {
      ...
      expect(getElementTextBySelector<NowPlayingComponent>(fixture, '.minutes-until-sleep')).toBe(
        `Sleeping in ${i} minutes`
      );
    }
